...
function registerAiRoutes(app, deps) {
  ...
  app.post('/api/questions/cache/check', authMiddleware, async (req, res) => {
    ...
    try {
      ...
      if (queryText !== 'gorsel_icerik' && shouldUseSemanticCache(course, queryText, semanticImageBlocker)) {
      ...
      if (queryText !== 'gorsel_icerik' && shouldUseEmbeddingCache(course, queryText, embeddingImageBlocker)) {
  ...
  app.post('/api/ai/ask', authMiddleware, async (req, res) => {
    const { prompt, course, systemInstruction, history, base64Image, imageMimeType } = req.body;
    ...
    try {
      ...
      await writeTelemetry(prisma, {
        ...
        isImage: Boolean(base64Image),
        usedOcr: false,
        estimatedCostUsd: estimateRequestCostUsd({
          isImage: Boolean(base64Image),
        }),
