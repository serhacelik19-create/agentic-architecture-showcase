const SUPER_ADMIN_USERNAME = process.env.SUPER_ADMIN_USERNAME || 'serhat';
const SUPER_ADMIN_PASSWORD = process.env.SUPER_ADMIN_PASSWORD || '1';
const SUPER_ADMIN_NAME = process.env.SUPER_ADMIN_NAME || 'Serhat Bey';
  ...
  app.post('/api/login', loginLimiter, validate(loginSchema), async (req, res) => {
    ...
    try {
      if (username === SUPER_ADMIN_USERNAME && password === SUPER_ADMIN_PASSWORD) {
        const token = generateToken({ id: 0, role: 'super_admin', name: SUPER_ADMIN_NAME });
        ...
        return res.json({
          success: true,
          user: { name: SUPER_ADMIN_NAME, role: 'super_admin', avatar: 'S' },
          token,
        });
      }
      ...
      if (user && user.password) {
        ...
        if (isMatch) {
          ...
          return res.json({
            success: true,
            user: { ...safeUser, avatar: user.name?.charAt(0) || 'A' },
            token,
          });
        }
      }
